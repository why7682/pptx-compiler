...
| M4-001B | DONE | Preserve the completed immutable `alpha.1` candidate/tag gates as retired unpublished history, then admit the fresh D-049 `0.1.0-alpha.2` package projection, dual-builder lock, reviewed merge `S2`, single-parent attestation `A2`, exact history relation, annotated tag, and tag-hosted gates. | Node 22.23.2/npm 10.9.8 and Node 24.19.0/npm 11.17.0 produced equal canonical payloads; the tracked lock, verified `S2`, sole-child exact-grant `A2`, annotated `v0.1.0-alpha.2 -> S2`, and tag Public CI/Security were admitted before npm publication. |
| M4-001C | BLOCKED | Preserve the exact partial `alpha.2` registry prefix after attempt 2 published core and stopped; never complete that version through tag movement, repack, unpublish, or dist-tag repair. | Exact reviewed core bytes and later certificate-bound provenance are verified; the other three versions and GitHub Release remain absent; both old tags/locks and the registry write are immutable. The former no-`latest` exit condition is impossible because npm package metadata requires `latest`. |
| M4-001D | DONE | Implement and execute D-050's fresh `0.1.0-alpha.3` forward recovery with registry seed-`latest` semantics and bounded provenance propagation. | A fresh dual-builder lock and `S3 -> A3`/annotated-tag/hosted chain preceded publication; exact npm publish operations yielded core `alpha -> alpha.3` with frozen `latest -> alpha.2`, and the three first-created identities yielded `alpha/latest -> alpha.3`; all bytes and certificate-bound attestations passed, and the GitHub prerelease was last. A separate local interactive npm 11.17.0 procedure then fresh-read four exact Trusted Publisher settings; only afterward did the operator uniquely revoke the bootstrap token by full ID and fresh-prove it absent, then delete and fresh-prove absent only the `npm-release` environment's `NPM_TOKEN` in `why7682/pptx-compiler`. Configured/visible is not execution proof; only a future tokenless OIDC publish proves the bindings. No separate `npm dist-tag add/rm` or unpublish occurred. |
...
### M4-001 D-048 authorization checkpoint (historical) — 2026-08-12

This subsection preserves the exact model used at that dated checkpoint. Its
no-`latest` expectation is not a current instruction: live registry evidence
invalidated it, and D-050 plus M4-001D now own the seed-`latest` recovery model.
...
### M4-001B `alpha.1` candidate admission and pre-publish stop — 2026-08-13

- GitHub merged the reviewed lock as verified source commit
  `S1=ad4ab94959e9f7cff56834c81be4ddecd11e7332`. The repository-local
  public-identity attestation
  `A1=7d8341ebc01f8769a73020103d18c93b4049468f` has `S1` as its sole parent,
  exact-grants `S1`, passed the complete reachable-history gate, and became
  remote `main`.
- Annotated tag `v0.1.0-alpha.1` has immutable tag object
  `0d23ce3903052454ba77e6c88360f0c4fffa4173` and peels to `S1`. Tag Public CI
  run `31652084833` and Security run `31652084727` passed. This completes the
  `alpha.1` source/lock/tag-hosted portion of M4-001B.
- Manual Alpha npm Release run `31652404999` then passed immutable checkout,
  source/history/workflow policy, complete tests, guarded package build,
  candidate/tag admission, and exact remote-main verification. In the first
  publish step, the OIDC environment gate rejected GitHub's legitimate sharded
  `ACTIONS_ID_TOKEN_REQUEST_URL` because the implementation required one
  previously observed hostname.
- The failure was before the first `npm publish`; the GitHub declaration job
  was skipped. Official-registry rereads found all four exact package
  packuments/versions absent, and the GitHub Release was absent. No rollback,
  unpublish, tag movement, or destructive reconciliation occurred.
- `0.1.0-alpha.1` is retired unpublished. D-048, its lock, note, `S1`, `A1`,
  tag, and hosted runs remain immutable history and cannot authorize replacement
  bytes.

### M4-001B `alpha.2` recovery authorization — 2026-08-13

- D-049 records the user's explicit authorization for all three replacement
  effects: annotated tag and GitHub prerelease `v0.1.0-alpha.2`; the same four
  public packages at `0.1.0-alpha.2` under dist-tag `alpha`; and GitHub Actions
  npm provenance followed by all four Trusted Publisher bindings, bootstrap-
  token revocation, and GitHub `NPM_TOKEN` deletion.
- The real defect is the overly fixed OIDC-host representation. The smallest
  correction admits HTTPS on exactly one validated DNS label beneath
  `actions.githubusercontent.com` while retaining rejections for user info,
  ports, fragments, empty paths, nested labels, suffix confusion, non-ASCII raw
  URL text, and non-GitHub authorities. The later fetched provenance bundle
  must still pass exact workflow/tag certificate and issuer verification.
- Recovery starts from a fresh version projection. It must create a new
  lifecycle-neutral `alpha.2` note, dual-builder package results, create-only
...
### M4-001B completion and M4-001C partial publication — 2026-08-14

- GitHub-verified `S2=b884b39bdded17d7bc2ccedad159605523329bae`
  contains the unchanged reviewed `alpha.2` lock. Its repository-local
  attestation `A2=626560ec43e748ac8002352d9f849ee0d6e09b2f` has `S2` as sole parent and
  exact-grants it. Annotated tag object
  `e7f6f17b7ba2db6ef723d309ed35ea7dd1ccef1d` peels to `S2`; tag Public CI
  `31665054553` passed all six cells and Security `31665054531` passed CodeQL.
  M4-001B is complete.
- Manual release run `31665307969`, attempt 2, published only
  `pptx-compiler-core@0.1.0-alpha.2`, then stopped around
  `2026-08-13T19:43:35Z` during immediate attestation verification. The
  official tarball is 118,488 bytes with SHA-256
  ...
  exactly matching the fixed Node 24/npm 11 lock envelope.
- The packument records publish time `2026-08-13T19:43:34.408Z` and both
  `alpha` and registry-required `latest` at `0.1.0-alpha.2`. A later independent
  read matched the keyless SLSA v1/v0.3 subject, `S2`, tag, workflow, issuer,
  and attempt-2 invocation; the fixed npm 11.17.0/Sigstore 4.1.1 verifier and
  isolated `npm audit signatures --include-attestations` passed with no invalid
  or missing entry. The original workflow did not record that later success.
- A later attestation response carried `Last-Modified: 19:44:33 GMT`, roughly
  59 seconds after publish metadata. That is an observed propagation interval,
  not an npm SLA or exact first-visible time; the suppressed child error cannot
  distinguish absent, empty, or cryptographically incomplete first state.
- The other three `alpha.2` versions and its GitHub Release remain absent. No
  unpublish, Git-tag movement, separate `npm dist-tag add/rm`, or GitHub
  declaration occurred. M4-001C is preserved as `BLOCKED` rather than repaired
  destructively or mislabeled complete.

### M4-001D `alpha.3` recovery authorization — 2026-08-14

- D-050 records the user's explicit authorization for the fresh annotated tag
  and matching GitHub prerelease, all four packages at exact
  `0.1.0-alpha.3` under `alpha` with GitHub Actions npm provenance, and the
  existing Trusted Publisher/bootstrap-credential transition. D-048/D-049 and
  every old tag, lock, version, run, and registry write remain unchanged.
- The corrected registry model separates package identity, immutable version,
  and package-level tags. The official metadata contract requires every package
  to have `latest`. Core must therefore retain `latest -> 0.1.0-alpha.2` while
  its exact `alpha.3 --tag alpha` publish assigns `alpha -> alpha.3`; the other
  three first-created identities must receive registry-seeded `latest` and
  requested `alpha` at `alpha.3`. No separate dist-tag mutation is authorized.
- Before any external write, implement and verify bounded exact-state retries
  for packument/tarball/attestation propagation, then create the complete fresh
  package projection, lifecycle-neutral note, two builder results, lock,
  `S3 -> A3`, annotated tag, and tag-hosted evidence. Synchronous mocks verify
  control flow but do not substitute for current upstream research or live
  official-registry observations.
- After the four identities exist and the GitHub Release succeeds, the existing
  `alpha-release.yml` workflow stops; it does not automatically change npm or
  GitHub settings. Run a local interactive npm 11.17.0 procedure for each exact
  package, beginning with fresh `npm trust list <exact-package> --json`. Empty
  permits one create, exactly one normalized exact binding permits an
  idempotent rerun, and any mismatch or other cardinality hard-stops. Normalize
  raw `type: "github"` to provider `github-actions`, raw `file:
  "alpha-release.yml"` to the workflow filename, and sole `permissions:
  ["createPackage"]` to allowed action npm publish; reject
  `createStagedPackage`. Owner `why7682`, repository `pptx-compiler`, and
  environment `npm-release` must also match.
- Only after four fresh exact binding readbacks, take a fresh
  `npm token list --json`; the operator uniquely confirms the bootstrap token's
  full ID or hard-stops on ambiguity, revokes only that ID, and fresh-proves it
  absent. Then delete only the GitHub Actions environment secret `NPM_TOKEN`
  from repository `why7682/pptx-compiler`, environment `npm-release`, and
  fresh-prove that exact name absent. Record no secret values. Configuration
  visible on npm is not execution proof; a future tokenless OIDC publication is
  the first proof that the binding works.

### M4-001D `alpha.3` create-only lock checkpoint (historical) — 2026-08-14
...
- At that historical checkpoint, the untracked lock was 6218 bytes, mode
  `100644`, with SHA-256
...
- Independent no-local/no-hardlink regeneration and direct Git-tree/tar
  recomputation reported 0 blocker, 0 high, and 0 medium with Taste=good. They
  admitted only the exact mode-`100644`, 6218-byte lock at Git blob
  `eb875526ffefc81b4bbaa2c15ed4412b31a8d026` for tracking; it was still
  untracked and any byte change required a new dual-builder and independent
  review. M4-001D stayed `IN_PROGRESS`. No `S3`, `A3`, annotated tag,
  tag-hosted result, npm `alpha.3` package, registry provenance/signature or
  dist-tag result, GitHub Release, Trusted Publisher binding, bootstrap-token
  revocation, or environment-secret deletion existed at that checkpoint.

### M4-001D completion — 2026-08-14

- The preceding create-only checkpoint remains historical. GitHub subsequently
  merged the unchanged lock as verified
  `S3=20f7f64faa8c8d688922896296c134b25bc58e7f`; exact-grant attestation
  `A3=7270ae7814583117050abac648ba96067e4fce67` is its sole child. Annotated tag
  object `46c5360bd5daa48a1b493f1c9310b2358b3d6e6d` peels to `S3`.
- A3 Public CI/Security runs `31750359756`/`31750359778` passed. Tag Public
  CI/Security runs `31750881903`/`31750881914` passed on unchanged `S3`.
- Release run `31751437354` failed closed twice without rollback or a second
  write after an uncertain result. Attempt 1 left only exact
  `pptx-compiler-core@0.1.0-alpha.3` present after bounded propagation
  exhausted; attempt 2 stopped at the GitHub-repository precondition before
  another npm write. A no-write cooldown and two time-separated exact official
  reads preserved that one-package prefix.
- Fresh release run `31756489430` then passed admission, completed the remaining
  dependency-order registry suffix, reverified all four official tarballs,
  certificate-bound provenance/signatures, and complete package-level tag maps,
  and created GitHub Release `370278133` last. The non-draft prerelease has tag
  `v0.1.0-alpha.3`, target `S3`, and no assets.
- The official Node 24/npm 11 envelopes are exact: core 118555 bytes / SHA-256
  `60c9466bff289b72e88752bd21658f170c33d04740aac3e7cbfe1a823784905d`;
  native 11012 / `d66da46daf4369a51b252f971205465393ffd827a1908669d9a44dba2bfaeb63`;
  public 21242 / `17cc4300f012e1a93b745c7e91f6ec1a05dc3232566166f9253054d447f5358a`;
  CLI 13829 / `a3713bd6d88628148271b9122efbf7dce7d41d1a1120220fb094f6488cc30cb8`.
  Core has `alpha -> 0.1.0-alpha.3` and frozen
  `latest -> 0.1.0-alpha.2`; each other package has both `alpha` and `latest`
  at `0.1.0-alpha.3`.
- A separate local interactive npm 11.17.0 procedure fresh-read exactly one
  normalized Trusted Publisher binding for each package: provider
  `github-actions`, owner `why7682`, repository `pptx-compiler`, workflow
  `alpha-release.yml`, environment `npm-release`, and allowed action npm publish
  only. After all four reads, the operator uniquely revoked the bootstrap token
  by full ID and fresh-proved it absent, then deleted and fresh-proved absent
  only the `npm-release` environment secret `NPM_TOKEN`. No ID or secret value is
  recorded. These bindings are configured/visible, not tokenless OIDC execution
  proof.
- No unpublish, old-tag movement, version reuse, separate dist-tag mutation, or
  destructive reconciliation occurred. `alpha.1` and `alpha.2` remain immutable
  history, support remains 0/22/3/35 with its global switch false, and
  D-047/M3-008 remains deferred.
