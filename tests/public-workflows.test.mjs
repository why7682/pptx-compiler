...
test("ordinary CI/security and the isolated alpha release workflow are canonical", () => {
  ...
  assert.equal(ALPHA_RELEASE_TAG, "v0.1.0-alpha.3");
  assert.equal(ALPHA_RELEASE_ENVIRONMENT, "npm-release");
  assert.equal(ALPHA_RELEASE_PREPARATION_COMMAND,
    "node scripts/check-alpha-release-preparation.mjs --mode release-tag --tag v0.1.0-alpha.3 --stage-root .package-stage/reviewed");
  assert.equal(ALPHA_RELEASE_SOURCE_VERIFICATION_COMMAND,
    "node scripts/publish-alpha-release.mjs --mode verify-source --tag v0.1.0-alpha.3 --stage-root .package-stage/reviewed");
  assert.equal(ALPHA_RELEASE_PUBLISH_COMMAND,
    "node scripts/publish-alpha-release.mjs --mode publish --tag v0.1.0-alpha.3 --stage-root .package-stage/reviewed");
  assert.equal(ALPHA_GITHUB_RELEASE_COMMAND,
    "node scripts/create-alpha-github-release.mjs --tag v0.1.0-alpha.3 --stage-root .package-stage/reviewed");
...
test("workflow mutations fail closed without a general YAML parser", async (t) => {
  ...
  await t.test("release rejects a branch ref, automatic trigger, or broader target", () => {
    ...
    const releaseMutation = `${original}      - run: gh release create v0.1.0-alpha.3\n`;
    assert.equal(inspectPublicWorkflowText(
      ".github/workflows/alpha-release.yml",
      releaseMutation
